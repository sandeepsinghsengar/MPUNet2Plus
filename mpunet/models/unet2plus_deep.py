...
class UNet2Plus_up_deep(Model):
    ...
    def __init__(self,
                 ...
                 logger=None,
                 **kwargs):
        super(UNet2Plus_up_deep, self).__init__()
    ...
    def init_model(self):
    	...
    	x4_0 = self.conv_block_nested(self.pool(x3_0),filters[4], filters[4])
    	x3_1 = self.conv_block_nested(Concatenate(axis=-1)([x3_0, self.Up(x4_0)]),filters[3], filters[3])
    	x2_2 = self.conv_block_nested(Concatenate(axis=-1)([x2_0, x2_1, self.Up(x3_1)]),filters[2], filters[2])
    	x1_3 = self.conv_block_nested(Concatenate(axis=-1)([x1_0, x1_1, x1_2, self.Up(x2_2)]),filters[1], filters[1])
    	x0_4 = self.conv_block_nested(Concatenate(axis=-1)([x0_0, x0_1, x0_2, x0_3, self.Up(x1_3)]),filters[0], filters[0])
    	
    	out1 = Conv2D(self.num_class, kernel_size=1,activation='softmax',name='final_layer1')(x0_1)
    	out2 = Conv2D(self.num_class, kernel_size=1,activation='softmax',name='final_layer2')(x0_2)
    	out3 = Conv2D(self.num_class, kernel_size=1,activation='softmax',name='final_layer3')(x0_3)
    	out4 = Conv2D(self.num_class, kernel_size=1,activation='softmax',name='final_layer4')(x0_4)
    	out=(out1+out2+out3+out4)/4
    	if self.flatten_output:
            out = Reshape([self.img_shape[0]*self.img_shape[1],
                           self.num_class], name='flatten_output')(out)
